...
<div style="text-align: right; margin-top: 10px;margin-right: 20px;">
	<c:choose>
		<c:when test="${dlist.pageNo>dlist.topPageNo}">
			<a href="message?pageNo=${dlist.topPageNo}">第一页</a>
			<a href="message?pageNo=${dlist.previousPageNo}">上一页</a>
		</c:when>
		<c:otherwise>
		首页　上一页
</c:otherwise>
	...
	<c:choose>
		<c:when test="${dlist.pageNo<dlist.bottomPageNo}">
			...
			<a href="message?pageNo=${dlist.nextPageNo}">下一页</a>
			<a href="message?pageNo=${dlist.bottomPageNo}">最后页</a>
		</c:when>
		<c:otherwise>
下一页　尾页
</c:otherwise>
